...
def prep(points):
    ...
    l5 = np.sqrt(np.sum((points[:,4] - points[:,3])**2,1))
    l6 = np.sqrt(np.sum((points[:,4] - np.array([0.375, 0]))**2,1))
    ...
    v5dv6 = v5.reshape(len(points),1,2)@v6
    ...
    L = l1 + l2 + l3 + l4 + l5 + l6
    rs = len(points)
    trainer = np.concatenate([(l1/L).reshape(rs,1), ((l1+l2)/L).reshape(rs,1), ((l1+l2+l3)/L).reshape(rs,1),((l1+l2+l3+l4)/L).reshape(rs,1),((l1+l2+l3+l4+l5)/L).reshape(rs,1), ((theta1+np.pi)/(2*np.pi)).reshape(rs,1), ((theta2+np.pi)/(2*np.pi)).reshape(rs,1) , ((theta3+np.pi)/(2*np.pi)).reshape(rs,1), ((theta4+np.pi)/(2*np.pi)).reshape(rs,1), ((theta5+np.pi)/(2*np.pi)).reshape(rs,1)],1)
