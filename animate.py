...
save = {'P':[], 'feat':[], 'knots':[], 'tup':[], 'inte':[], 'cknots':[],'cinte':[]}
history = []
ani = 0
...
fig2,axes2 = plt.subplots(1,1,figsize=(9.0,8.0),sharex=clamp,subplot_kw=dict(projection='3d'))
ax2 = axes2
...
save['P'].append(P)
save['feat'].append(trainer)
save['knots'].append(knots)
save['cknots'].append(cknots)
save['tup'].append((X,Y,Z))
save['inte'].append(inte)
save['cinte'].append(cinte)
...
def update():
    ...
    text6.set_text(str(knots[4:6]))
    text8.set_text(str(cknots[4:6]))
    text10.set_text(str(trainer))
    ...
    th2 = ax2.scatter(knots[4],knots[5],np.log10(inte),alpha=1,c='k') #mod
    th3 = ax2.scatter(cknots[4],cknots[5],np.log10(cinte),alpha=1,c='b') #cent
    save['P'].append(P)
    save['feat'].append(trainer)
    save['knots'].append(knots)
    save['cknots'].append(cknots)
...
def reset(event):
    ...
    # th2 = ax2.scatter(knots[4],knots[5],np.log10(inte),c='k') #mod
    # th3 = ax2.scatter(cknots[4],cknots[5],np.log10(cinte),c='b') #cent
    th = ax2.plot_surface(Yc, Xc, Zc, rstride=1, cstride=1,
                cmap='viridis',alpha=0.6, edgecolor='none')
...
def change_plot(frame_number, zarray, tup):
    global th
    global th2
    global th3
    ...
    th = ax2.plot_surface(Y, X,  zarray[:, :, frame_number], rstride=1, cstride=1,
                cmap='viridis',alpha=0.6, edgecolor='none')
    th2 = ax2.scatter(knots_list[frame_number][0].detach().numpy(),knots_list[frame_number][1].detach().numpy(),np.log10(curv_list[frame_number]),alpha=1,c='k') #mod
    th3 = ax2.scatter(cknots_list[frame_number][0],cknots_list[frame_number][1],np.log10(ccurv_list[frame_number]),alpha=1,c='b') #cent
    # th3 = ax2.scatter(cknots[4],cknots[5],np.log10(cinte),alpha=1,c='b') #cent
...
def animate(event):
    ...
    # zarray = np.zeros(save['tup'][0].shape[0], save['tup'][0].shape[0], len(save['tup']))
    # frn = len(save['tup'])
    # for i in range(frn):
    #     zarray[:, :, i] = tup[i][2]
    ...
    PP = np.copy(P)
    X,Y,Z = tdplot(PP)
    zarray = np.zeros_like(Z)
    zarray = np.expand_dims(zarray,axis=2)

    curv_list = []
    ccurv_list = []
    knots_list = torch.zeros(1,2)
    knots_list.type(torch.DoubleTensor)
    cknots_list = torch.zeros(1,2)
    cknots_list.type(torch.DoubleTensor)
    ...
    for i in history:
        ...
        zarray = np.concatenate([zarray,Z],axis=2)
        trainer,c = prep(PP[1:3].reshape(1,2,2))
        ...
        knots = net((trainer,c))
        knots_list = torch.cat((knots_list, knots[:,4:6]),dim=0)
        #knots_list = knots_list[1:]

        cknots = centripetal(PP)
        cknots_list = np.concatenate((cknots_list, np.expand_dims(cknots[4:6],axis=0)),axis=0)
        #cknots_list = cknots_list[1:]


        knots = knots[0].detach().numpy()
        print(knots)

        C = interpolation.solve(PP, knots) #changed 5
        inte = float(curvature.global_curvature(C, knots, 3))
        curv_list.append(inte)

        #cknots = cknots[0].detach().numpy()
        print(cknots)
        C = interpolation.solve(PP, cknots) #changed 5
        cinte = float(curvature.global_curvature(C, cknots, 3))
        ccurv_list.append(cinte)
    ...
    zarray = zarray[:,:,1:]
    print(zarray.shape)
    ani = animation.FuncAnimation(fig2, change_plot, len(history), fargs=(zarray, (X,Y,knots_list,cknots_list,curv_list,ccurv_list)), interval=1000 /100)
    plt.show()
...
def motion_notify_callback(event):
    ...
    # if event.inaxes and not pind is None and event.button == 1:
    if event.button==1:
        if pind == 1:
            pind1 = history[-1][0]
            pind1x = pind1[0]
            pind1y = pind1[1]
            x = np.linspace(pind1x,event.xdata,21)
            x = x[1:-1]
            y = pind1y + (x-pind1x)*(event.ydata-pind1y)/(event.xdata-pind1x)
            toappend = [[[x[i],y[i]],[P[2][0][0],P[2][1][0]]] for i in range(len(y))]
            history = history + toappend
            history.append([[event.xdata, event.ydata],[P[2][0][0],P[2][1][0]]]) 
        else:
            pind2 = history[-1][1]
            pind2x = pind2[0]
            pind2y = pind2[1]
            x = np.linspace(pind2x,event.xdata,21)
            x = x[1:-1]
            y = pind2y + (x-pind2x)*(event.ydata-pind2y)/(event.xdata-pind2x)
            toappend = [ [[P[1][0][0],P[1][1][0]],[x[i],y[i]]] for i in range(len(y))]
            history = history + toappend            
            history.append([[P[1][0][0],P[1][1][0]],[event.xdata, event.ydata]]) 
...
cm, = ax1.plot (cF[0], cF[1], 'b-', label='cent. B-Spline') #Curve that will be updated.
...
th2 = ax2.scatter(knots[4],knots[5],np.log10(inte),alpha=1,c='k',label='model') #mod
th3 = ax2.scatter(cknots[4],cknots[5],np.log10(cinte),alpha=1,c='b',label='centripetal') #cent
...
axres2 = plt.axes([0.84, 0.7-((4)*0.05), 0.12, 0.02])
bres2 = Button(axres2, 'animate')
bres2.on_clicked(animate)
...
text3 = ax1.text(0.82, 0.68, "centripetal Curvature", transform=fig.transFigure)
text4 = ax1.text(0.82, 0.66, str(cinte), transform=fig.transFigure)

text5 = ax1.text(0.82, 0.64, "mknots", transform=fig.transFigure)
text6 = ax1.text(0.82, 0.62, str(knots[4:6]), transform=fig.transFigure)

text7 = ax1.text(0.82, 0.60, "cknots", transform=fig.transFigure)
text8 = ax1.text(0.82, 0.58, str(cknots[4:6]), transform=fig.transFigure)

text9 = ax1.text(0.45, 0.05, s="features", transform=fig.transFigure)
text10 = ax1.text(0.45, 0.02, str(trainer), transform=fig.transFigure)
